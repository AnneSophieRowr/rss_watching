class DatetimepickerInput < SimpleForm::Inputs::Base
  ...
  def input
      ...
      date = template.content_tag :div, class: 'col-xs-7 input-group pull-left datepicker' do
        field = @builder.text_field(attribute_name, input_html_options.merge(class: 'date_picker form-control', value: (value.nil? ? Date.today : value.try(:to_date))))
      ...
      time = template.content_tag :div, class: 'col-xs-4 input-group timepicker' do
        field = @builder.text_field(attribute_name.to_s.gsub('date', 'time'), input_html_options.merge(class: 'form-control', value: (value.nil? ? Time.now.strftime('%H:%M') : value.try(:strftime, '%H:%M'))))
        field += template.content_tag :span, '', class: 'input-group-addon' do 
